...
function plot_upset(my_dict)
    ...
    begin
        ...
        for c in combinations(collect(keys(my_dict)))
    ...
    blank_plot1 = plot(framestyle = :none, grid = false, xlims = (0, 10), ylims = (0, 10), legend = false, axis = false, ticks = false)
    ...
    combined_plot_side =plot(blank_plot1, barplot_number, label_plot, dot_plot, layout=(2,2), link= :both)
    
    for i in 1:height_y
        if i%2 == 0
            plot!(rectangle(length_x+0.5,1,0,i-0.5), subplot=4, opacity=0.5, color=:lightgray, linewidth=0)
        end
    end
    
    for i in 1:length_x
        plot!( [i,i], [0.75, height_y], subplot=4, opacity=0.3, linecolor=:black, linewidth=4)
    end
    
    
    for i in 1:length(filtered_intersection_lengths)
        annotate!(i, filtered_intersection_lengths[i] + 5.0, subplot=2, text(filtered_intersection_lengths[i], :center))
    end
